// Proof-of-concept: renders Red directly from its Stick Nodes rig (rigs/Red.json) instead of a
// sprite frame, using the exact layout/draw math already validated in the sn-proto WASM prototype
// and the Android RigLayout/RigView port (see sn_proto_wasm_renderer and android_rig_renderer
// memory) - circle radius factor, circle-center-at-start-plus-radius, curve-chain smoothing for
// segment-built heads, and skipping thickness<=0 structural bones. Runs standalone, alongside the
// existing Shimeji-rendered characters - not wired into positioning/AI yet, just validating the
// renderer works inside a transparent always-on-top Electron window.
...
function draw(ctx, canvas, figure, pose) {
...
async function main() {
  ...
  const res = await fetch('rigs/Red.json');
