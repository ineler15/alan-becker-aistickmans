		...
		try {
			...
			if (getAction().hasNext()) {
				...
				try {
					...
					// Some authored behavior chains fail to build in certain
					// contexts (missing/invalid variables for that specific
					// transition). Falling back to Fall instead of killing
					// the mascot keeps it alive and visible instead of
					// popping an error dialog and disposing it.
					log.log(Level.WARNING, "Failed to build next behavior, falling back to Fall (" + this + ")", e);
					try {
						getMascot().setBehavior(this.getConfiguration().buildBehavior(BEHAVIORNAME_FALL));
					} catch (final BehaviorInstantiationException e2) {
						throw new CantBeAliveException( Main.getInstance( ).getLanguageBundle( ).getString( "FailedInitialiseFollowingActionsErrorMessage" ), e2);
					}
				}
			}
