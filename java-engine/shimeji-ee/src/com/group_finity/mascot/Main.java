...
public class Main
{
    ...
    /**
     * Image-set names that get controlled by the external AI ("brain") process instead of the
     * usual random XML-driven behavior selection - see com.stickmanai.AIBehavior. Add an entry
     * here (matching an img/&lt;name&gt; folder that's also listed in ActiveShimeji) to turn that
     * character into an independent AI-driven friend.
     */
    private static final java.util.Set<String> AI_DRIVEN_CHARACTERS = new java.util.HashSet<String>(
            java.util.Arrays.asList( "Red", "Orange", "Purple", "Green", "Blue", "Yellow", "AI", "TCO", "TDL", "victim" ) );
    ...
    public void createMascot( String imageSet )
    {
        ...
        try
        {
            if( AI_DRIVEN_CHARACTERS.contains( imageSet ) )
            {
                // This image set is controlled by the external AI ("brain") process
                // instead of the usual random XML-driven behavior selection.
                mascot.setBehavior( new com.stickmanai.AIBehavior( imageSet, imageSet ) );
            }
            else
            {
